...
bindTrust = (address) => {
  ...
  return new Promise((resolve, reject) => {
    let event = instance.ForgeTrust({trste: web3.eth.accounts[0]})
    event.watch((err, res) => {
      if(!err) {
        event.stopWatching()
        resolve(res.args)
...
Template.body.events({
  'click #btnTrust'(event) {
    ...
    if(!trustee_address) {
      ...
      createTrusteAgreement(trustee_address)
      .then((res) => {
        console.log(res)
      }, (err) => {
        console.log(err)
  ...
  'click #btnLookup'(event) {
    event.preventDefault()
    let truster_address = document.getElementById("Trust")
    if(!truster_address) {
      alert("Please enter an address!")
    } else {
